...
def generate_load_from_gc_log(gc_log_full_name):
    ...
    writer.write('No, GcType, young_release, young_promote, young_use_rate, old_use_rate, heap_use_rate, old_release\n')
    ...
    for row in reader.readlines():
        ...
        if re.match(GcInfoMessage.gc_log_young_reg, row):
            ...
            gc_items_dict['youngBeforeUsed'] = gc_items[4]
            gc_items_dict['youngAfterUsed'] = gc_items[5]
            gc_items_dict['youngAfterCommitted'] = gc_items[6]
            gc_items_dict['heapBeforeUsed'] = gc_items[7]
            gc_items_dict['heapAfterUsed'] = gc_items[8]
            gc_items_dict['heapAfterCommitted'] = gc_items[9]
            gc_items_dict['duration'] = gc_items[10]
            current_gc_info = GcInfoMessage(gc_items_dict)
        if re.match(GcInfoMessage.gc_log_full_reg, row):
            ...
            current_gc_info = GcInfoMessage(gc_items_dict)
        # 当前条gc日志的负载得分
        gc_load_dict = {'gcType': '', 'young_release': 0, 'young_promote': 0, 'young_use_rate': 0.0, 'old_use_rate': 0.0, 'heap_use_rate': 0,
                        'old_release': 0}
        ...
        gc_load_dict['young_use_rate'] = current_gc_info.rate_young
        gc_load_dict['old_use_rate'] = current_gc_info.rate_old
        gc_load_dict['heap_use_rate'] = current_gc_info.rate_heap
...
if __name__ == '__main__':
    gc_log_file = input('fanruan.gc.log文件: ')
