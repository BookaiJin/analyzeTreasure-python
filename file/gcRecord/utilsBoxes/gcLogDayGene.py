...
from file.gcRecord.utilsBoxes import gcLogUtils
...
def generate_day_log(gc_record_file):
    ...
    for row in gc_file_reader:
        gcRow = gcLogUtils.generate_gc_log(row)
        result_gc_log_file_writer.writerow(gcRow)

    gc_csv_file.close()
    analyzeFileUtils.sort_file_message(result_day_gc_log, ['node', 'gcStartTime'])
